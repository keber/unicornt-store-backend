...
| D9 | "Require branches up to date" | **Off on `qa` and `main`, on for `dev`** (both repos, 2026-09-05). The rule requires the *head* to contain the *base*'s tip — which assumes a feature branch derived from its base. A promotion train is the reverse: `qa` is derived from `dev`, so every promotion leaves `dev` permanently one commit behind `qa`, and the same for `qa` behind `main`. It deadlocks rather than merely nags, because both of GitHub's "Update branch" options are closed off — a merge commit onto `dev` violates `required_linear_history`, a rebase force-push violates `non_fast_forward`, and a squash back-merge does not help because the check is ancestry-based, not content-based. Kept on `dev`, where the head really is a feature branch. |
| D10 | Deployment gating | Backend `qa` requires a successful **`dev`** deployment, `main` requires a successful **`qa`** deployment (`required_deployments`). A commit cannot be promoted past an environment it never deployed to. Added to `main` 2026-09-05 — it had the rule on `qa` only, so prod could be promoted from a `qa` commit whose deploy had failed. The frontend has no equivalent: its prod path is GitHub Pages via `static.yml`, outside this pipeline (P8.4). |
