...
**Status:** complete, with one live-verification limitation noted (Swagger prod 404).
...
| Swagger — prod profile | **partial** — see note below |
...
springdoc `3.0.3` (the pre-release line built for Spring Boot 4) logs
`SpringDoc /v3/api-docs endpoint is enabled by default` and continues to serve the
endpoints under `prod` regardless of that flag. A belt-and-suspenders
`infrastructure/config/ProductionApiDocsGuard` — a `@Profile("prod")`
`OncePerRequestFilter` that answers `404` for every `/v3/api-docs*` / `/swagger-ui*` /
`/api-docs*` path before the dispatcher — was added and compiles/tests green.
A live prod-jar run to capture the 404 could not be completed in this environment:
repeated Windows file locks on `target/app.jar` during `spring-boot:repackage` left a
stale jar (the running prod jar predates the guard). Disposition: **configured +
guarded in code, hard live prod-404 evidence pending a clean jar build.** The dev
Swagger and both-profile CORS are verified live.
...
- **Swagger prod 404**: configured + code-guarded; live prod-jar 404 not captured
  (build-env instability). Recommend pinning springdoc to a release that honours
  `springdoc.api-docs.enabled=false`, or the `ProductionApiDocsGuard` filter carries it.
